...
import {
	createSlurmCourse,
	type CreateSlurmCourseOptions,
} from '@/lib/slurm-course-builder'
import {
	SlurmCourseStructure,
	type SlurmCourseLevel,
} from '@/lib/slurm-course-structure'
...
async function main() {
	...
	try {
		...
		const result = await createSlurmCourse({
			level,
			createdById: user.id,
			organizationId: undefined, // or provide organization ID if needed
		})
